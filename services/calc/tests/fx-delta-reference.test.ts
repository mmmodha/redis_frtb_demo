...
// Pure-TS oracle for FX Delta K_b — MAR21 §21.88–§21.91.
// A single risk factor per currency pair (bucket = currency pair, e.g. "EURUSD").
// All rows in the same bucket aggregate to ONE factor:
//   s_b  = Σ s_row
//   WS   = w · s_b
//   S_b  = WS                    (signed; used by cross-bucket reduce)
//   K_b  = |WS|                  (single-factor reduces ΣWS² + ρ·(0))
...
describe("computeKbFxDelta (TS reference oracle)", () => {
  it("returns |w · Σs| for a multi-row bucket", () => {
    const rows = [
    ...
    const expectedWS = W_FX * (1.0 + 2.5 - 0.5);
    expect(out.K_b).toBeCloseTo(Math.abs(expectedWS), 12);
    expect(out.S_b).toBeCloseTo(expectedWS, 12);
  ...
  it("skips non-finite risk_value rows", () => {
    ...
    expect(out.K_b).toBeCloseTo(3.0, 12);
