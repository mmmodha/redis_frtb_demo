...
describe("computeGirrCurvatureCharge (TS reference oracle)", () => {
  ...
  it("same-direction mixed-sign CVR pair exercises the ψ=1 branch with negatives present (verifier gap fix)", () => {
    // Single bucket, two tenors. cvr_up=[+2,-1], cvr_down=[-0.5,+1.5]. ρ_delta=0.5 ⇒ ρ²=0.25.
    // Every (k,l) pair has at least one non-negative ⇒ ψ=1 throughout (this is the gap
    // the verifier flagged — the ψ=0 branch was tested but the ψ=1-with-negatives branch
    // was not). Hand-computed expected values:
    ...
    const rows = [
      {
        sensitivity_type: "Curvature",
        bucket: "1",
        risk_value: { cvr_up: [+2.0, -1.0], cvr_down: [-0.5, +1.5] },
      },
    ];
    const out = computeGirrCurvatureCharge(rows, {
      tenors: 2,
