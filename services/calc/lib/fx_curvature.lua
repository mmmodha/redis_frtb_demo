-- frtb.fx_curvature — FX Curvature per-bucket K_b function.
...
-- owns that hash-tag.
--
-- Math (Basel MAR21 §21.5(2)–(3), FX Curvature; bucket-local):
--   CVR^{+|-} pre-computed scalar per row per §21.5(2); each row is a
--   distinct FX factor k (mirrors fx_delta.lua's per-row factor treatment).
...
--   squared value into __FX_CURVATURE_RHO__ (defaults to 0 → single-factor
--   specialisation K_b = max(|S_b^up|, |S_b^down|)).
...
-- FX curvature count semantics: only rows where both cvr_up and cvr_down
-- parse count (matches pre-refactor behavior).
...
redis.register_function('fx_curvature', function(keys, args)
