...
//
// Coordinator-driven Redis target swap endpoints for loadgen. Bearer-token
// guarded by INTERNAL_API_TOKEN. On prepare: stop in-flight workers (the
// Runner) so the new target inherits a quiet generator. On commit: caller-
// supplied callback (typically the active-target watcher's pollOnce)
// reconnects to the new target and we respond `phase=committed`. Both
// endpoints echo the incoming switch_id back for coordinator correlation.
