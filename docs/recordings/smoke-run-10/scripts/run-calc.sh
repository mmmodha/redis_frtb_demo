#!/usr/bin/env bash
set -u
LOG=docs/recordings/smoke-run-10/logs/calc-6variants.log
RESULTS=docs/recordings/smoke-run-10/calc-results.json
: > "$LOG"
echo "[]" > "$RESULTS"
...
try:
    ...
    body = body_raw
out_fp = 'docs/recordings/smoke-run-10/calc-results.json'
...
python3 -c "
import json
d=json.load(open('docs/recordings/smoke-run-10/calc-results.json'))
for r in d:
    body=r['body']
    if isinstance(body, dict):
        charge=body.get('sbm_charge','-')
        per_b=len(body.get('per_bucket') or [])
        print(f\"{r['risk_class']:7s} {r['sensitivity_type']:5s}  http={r['status']}  {r['wallclock_ms']}ms  charge={charge}  per_bucket={per_b}\")
    else:
