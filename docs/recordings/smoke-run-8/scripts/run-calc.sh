#!/usr/bin/env bash
set -u
LOG=docs/recordings/smoke-run-8/logs/calc-6variants.log
RESULTS=docs/recordings/smoke-run-8/calc-results.json
: > "$LOG"
echo "[]" > "$RESULTS"
...
try:
    ...
    body = body_raw
out_fp = 'docs/recordings/smoke-run-8/calc-results.json'
...
python3 -c "
import json
d=json.load(open('docs/recordings/smoke-run-8/calc-results.json'))
for r in d:
    body=r['body']
