...
## Step 11a — Refine to a bucket subset (storyboard Beat 11) (1 min)

- **💼 Business value:** Risk officers can ask "what does this charge look like over just these three currency buckets?" and get the answer in single-digit-ms-per-bucket fanout because the bucket subset is pushed down into the discovery index — the api never enumerates buckets it does not need and never dispatches FCALLs for them. *Capabilities used: RediSearch `FT.AGGREGATE` TAG filter push-down at the discovery layer; same map-reduce code path.*
- **Narration:** "Same data, same kernels — I click three of the eleven bucket pills off. Calculate. Look at the `fanout_ms` badge — it just dropped from ~174 ms to under 50 because we fanned out three FCALLs instead of eleven. Watch the commands panel: the discovery query now reads `@risk_class:{GIRR} @bucket:{CAD|SEK|JPY}` verbatim. No api branch, no second code path — it's a TAG predicate on the Search index."
- **Objection:** "Couldn't your app filter that bucket list itself before dispatching?"
- **Rebuttal:** "It could — but then every shard would still enumerate every bucket on its slice, the api would still pay the discovery round-trip for the full set, and you would lose the slot-affinity guarantee on the narrowed list. Push-down at the index means the work disappears at the data layer, not in middleware. Same shape on a multi-shard cluster: each shard only enumerates its own slice of the requested buckets."
- **Fallback:** If the pills do not render (no prior Calculate), run the default GIRR Delta first to populate the per-bucket panel, then narrow. If `fanout_ms` does not visibly drop, narrate the discovery query change in the commands panel — that is the unambiguous on-screen evidence.

## Step 11b — Toggle the correlation regime: Low → High (storyboard Beat 12) (1 min)

- **💼 Business value:** Risk officers can stress-test the cross-bucket correlation regime live without a schema reload, a service restart, or a code change — exactly the "what if correlations move 25 % under stress?" question that Basel §21.6 puts in front of the desk. *Capabilities used: in-kernel ρ/γ scaling on the existing γ lookup; same reduce path; observability surfaces the applied factor verbatim.*
- **Narration:** "Basel §21.6 lets the bank scale the cross-bucket γ matrix to model regime shifts. Med is our default — γ × 1.0, the reference number. Click Low — γ × 0.75, the charge drops because the cross-bucket reinforcement weakens. Click High — γ × 1.25 with each ρ_bc capped at 1.0, the charge climbs. For Curvature the scaler lands *before* the γ² operator, so the swing is more pronounced. No schema swap, no redeploy."
- **Objection:** "We already scale γ in our spreadsheet model — what does in-database give us?"
- **Rebuttal:** "Your spreadsheet scales γ after the K_b values are out of the data store; we scale γ at reduce time *inside* the same call that produced the K_b values, so the desk sees the regime swing with sub-second turnaround and the api response carries the factor and the ρ-cap note verbatim for audit. The schema YAML is untouched — the regime is a request-time selector, not a configuration change."
- **Fallback:** If the segmented control does not respond to keyboard nav, click it with the mouse — both paths exit through the same `onChange`. If the charge does not visibly shift between Low and High, expand the commands panel and read the `regime.factor` and `regime.note` fields aloud — those are the wire-level proof.

## Step 11c — Exclude one risk-factor pool (storyboard Beat 13) (1 min)

- **💼 Business value:** Risk officers can ask "show me the charge *without* this synthetic risk-factor that's flagged for review" and the answer arrives in the same round-trip because excluded rows are skipped inside the Lua kernel, on the slot owning the data, before they contribute to `K_b` or `S_b`. *Capabilities used: kernel-side predicate push-down via the `<exclude_*_csv>` positionals on the FCALL arg_template.*
- **Narration:** "Last filter — pool-level exclusion. I add `RF_GIRR_05` to the Advanced filters exclude list. Calculate. The K_b for the affected buckets dropped because every row tagged with that factor was skipped *inside* the FCALL — that's the third exclude positional you see on the arg_template in the commands panel: `… GIRR <bucket> '' '' RF_GIRR_05`. Row gating happens on the shard, before the row ever lands in the accumulator. No second pass, no client-side post-filter."
- **Objection:** "We can post-filter the result in our app."
- **Rebuttal:** "Post-filtering re-aggregates wrong: SBM is non-linear in `K_b`, so dropping a row at the app tier means re-doing the per-bucket sum without that row — which means shipping every row out of the store anyway. Predicate push-down inside the Lua kernel means the excluded rows never leave the shard, the per-bucket `K_b` is computed correctly the first time, and the network footprint is identical to a non-filtered call."
- **Fallback:** If the SuggestCombobox does not produce a `RF_GIRR_05` suggestion, type the full value and press Enter to commit as free text — the kernel does not care whether the value came from the suggester or was typed verbatim.

## Step 11d — Typeahead in the filter inputs (storyboard Beat 14) (30 s)

- **💼 Business value:** Risk officers can autocomplete a 100 000-trade book id or a long risk-factor name in well under 50 ms from the same operational store that holds the sensitivities — removing the second store (Elasticsearch / Solr) the bank would otherwise stand up just for typeahead. *Capabilities used: RediSearch `FT.SUGGET` dictionaries, populated at ingest by `FT.SUGADD … INCR` and backfilled on bootstrap.*
- **Narration:** "Same Redis cluster, same operational store. I start typing `RA` in the book filter — suggestions render in well under 50 ms because the Search engine has a purpose-built suggester index (`sug:book`, `sug:trade_id`, `sug:risk_factor`) seeded at ingest and live-populated by the consumer pipeline. No second store. No second index-on-write path. Day-1 your autocomplete is on the same cluster your charge is on."
- **Objection:** "We already run Elasticsearch for typeahead — why retire it?"
- **Rebuttal:** "Two stores means two ingest pipelines, two consistency models, two on-call rotations, and a window where the typeahead suggestion does not exist for a row your charge already includes. The Search engine inside Redis Enterprise gives you the same prefix-completion surface in the same place — one ack from the ingest consumer populates both the searchable row and the suggester entry."
- **Fallback:** If `/suggest` returns 503 (no suggester or dictionary empty), narrate the architecture against the slide and run `FT.SUGLEN sug:book` from a terminal tab — the count is the unambiguous proof the dictionary is populated even if the network round-trip is misbehaving on the live UI.
